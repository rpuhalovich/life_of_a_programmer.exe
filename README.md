...
On the bottom left of the Heads Up Display (HUD) you will see a dash meter where the two circles with triangles inside represent dashes currently able to be used (greyed out when unable) and the three arrows will fill up to recharge a used dash every one second. Also seen is a timer for the stopwatch that will begin counting up after passing through the first checkpoint and finish after reaching the final checkpoint (the bug). Your best time will be persistently recorded on your device and will be displayed under the current time on each run.
...
As stated, this allows for easily distinguishable gameplay elements. To name a few, only the grapple points are an emissive orange, boost pads are an emissive aqua, and the final bug is an emissive golden yellow to signal the final destination.

## The Graphics Pipeline and Camera Motion

The Graphics Pipeline remained relatively untouched save for the procedurally generated clouds, fog shader, distortion shader and the use of the URP for the built in post processing. The clouds and shaders will be discussed below.

As mentioned, the URP is utilised extensively to allow the use of various post processing effects. This gave us the ability to add a great amount of visual polish that would otherwise result in a very bland looking aesthetic. The most notable effects would be the chromatic aberration, bloom, color adjustments and depth of field. The chromatic aberration is used to give a distorted look to the edges of the screen, which is intended to give a sense of speed. Bloom is discussed previously and used extensively. Slight color adjustments are used to give a more cohesive colour palette. Depth of field is mostly notable when wallrunning and having the gun blur.
...
The Bug Glitch shader contains two types of glitches, the displacement glitch and the color glitch, both of which have a probability of happening which we’ve set to 0.1. We’ve only set both their intensities to 0.1. The shader also only has one subshader block.

It is important that the ‘Queue’ tag is set to ‘Transparent’ since the glitch effect is alpha-blended, allowing it to merge with the background when it glitches. The ‘PreviewType’ tag is set to ‘Plane’ as the shader is being used for a sprite. Culling is disabled, and the traditional transparency blend method, ‘SrcAlpha OneMinusSrcAlpha’ (also ideal for alpha-blending), is used.
...
The fog shader only has two properties; the color and intensity of the fog. It also only has one subshader block. Since the fog is also an alpha-blended effect, we set both the ‘Queue’ as well as the ‘RenderType’ tag to ‘Transparent’. Following the Bug Shader, we again use the traditional transparency blend method, ‘SrcAlpha OneMinusSrcAlpha’.

First, each vertex’s position on the screen is computed using ComputeScreenPos() and recorded as fog coordinates with UNITY_TRANSFER_FOG(). Then in the frag function, the depth is calculated for each vertex by getting its linear eye depth (depth buffer value in the world space) from a texture lookup on the camera’s depth texture using the input’s coordinates. Using the depth - the homogenous vertex coordinate (== 1.0 if the xyz coordinates are normalised) and multiplying that by the fog intensity, the resulting value is passed into the saturate function (clamped to the range [0,1]) to find the difference in RGB values of the fog and the original color of that point.

The output color is formed by interpolating the fog color and rather than just using the difference as the weight, a specific and “breathable” calculated smoothstep curve is used instead for a more smooth and pleasing color transition from the fog to the original color. Lastly we apply the output color to the fog coordinates we previously recorded.
...
The query technique employed was the Interview technique. What this entailed was that each participant was to play the game as much as they liked, on any of the four available levels (2_null_pointer_exception.lvl was not completed when the playtesting was conducted), of course starting from the tutorial. Afterwards, a series of questions were asked conversationally, the answers of which were recorded into a Google Doc. If at any point there was confusion we would assist if it is clear that they cannot progress without intervention.

We had seven participants take part in the query evaluation technique. All of which were in the 20 - 23 age range and were all to some degree regular gamers. Only one of which (Cynthia) had not played first person games before.
...
Given the context of skill levels for each of the participants, the game seemed to be challenging, but not to the point of not being fun. More experienced participants would be able to pick up the controls and complete levels easily while less experienced participants would require many more attempts to complete a level. This is not bad however, overall the difficulty was acknowledged to be slightly above average but not to a frustrating degree. The same was said by participants who were being evaluated using the Cooperative Evaluation technique below.

**_5: What do you think you would add to the game to improve it?_**

This was to be an open ended hypothetical question for any features or improvements that the participants wanted. Obviously, not all requests were able to be met with how out of scope some of them tended to be.
...
Romy had requested hidden alternate routes that would act as shortcuts for the more skillful players.

Brandon and Teo had both mentioned some sort of high score system. This would later manifest as the best time system.

### Observational

The observational technique employed was Cooperative Evaluation. Much like the querying technique, the participants were asked to play four levels of an early build of the game. They were then asked to speak their thoughts out loud as they played while we recorded dot point notes in a Google Doc. Cooperative Evaluation entails a reserved stance when observing participants, only intervening when something notable happens, or the participant fails to progress. Below is a summary of the major bits of feedback we received.
...
We had noticed that at the end of each level that many participants were confused after touching the final bug checkpoint as nothing substantial happens, as only the current timer stops counting up.

Some of the participants had managed to discover a bug that allowed them to grapple towards a grapple point through a wall. This bug is actually caused by the aim assist implementation, whereby the sphere colliders for the grapple points were much larger than the points themselves.
...
The grapple point implementation was not changed, rather the levels were edited slightly to prevent the sphere colliders from clipping through any surfaces.

A best time system was implemented to persistently record a players best time on their device, to be displayed every time they play.
...
Fog Shader: https://unitytutorials.ca/graphics/shaders/vertical-fog-shader
